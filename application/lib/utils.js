({
  ...
  flatten(obj, keys = []) {
    ...
    return Object.keys(obj).reduce((acc, key) => Object.assign(
      acc,
    ...
    for (const key of Object.keys(sourceObject)) {
      ...
      const tmpObjectPropValue =
        ...
        !lib.utils.isObjectID(value) && value !== null && typeof value === 'object' ?
          lib.utils.addDeepProxyChangesWatcher(value, [...path, key], storage).proxy : value;
  ...
  structuredClone(data, { convertFuncToString = false } = {}) {
    ...
    const replacer = convertFuncToString ? (key, value) => {
